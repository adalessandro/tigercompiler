...
fun P() = !Nlin.num_linea

fun nombre(SimpleVar s) = s
  | nombre _ = raise Fail "Imposible que no sea SimpleVar!"

(* fundeLFunTipos agrupa las declaraciones contiguas en un batch.
        Las declaraciones son de tipo o funcion *)
...
%type<Abs.ty> ty
%type<string> id
%type<Abs.field> tyfield
%type<Abs.field list> tyflds
%type<Abs.exp> exp
%type<Abs.exp list> explist
%type<(Abs.symbol*Abs.exp) list> rec_fields 
%type<Abs.exp list> args 
%type<var> l_value
%type<Abs.dec> dec
%type<Abs.dec> vardec
%type<Abs.dec> fundec 
%type<Abs.dec list> decs 

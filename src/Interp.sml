(*
    ...
    b: (Tree.stm list*Frame.frame) list - cada elemento de la lista es un par con la lista de Tree.stm devuelta por el canonizador y el frame de cada función
    c: (Temp.label*string) list - Una ista con un elemento por cada string definido en el código. Cada elemento es un par formado por el label y el string.
    inter usa: 
        Constantes de Frame: wSz, rv, fp
        Funciones de Frame: formals, exp. formals (Appel, pág 135) debe devolver una lista con los Frame.access de cada argumento pasado a la función, esto es el Frame.access que se usa en el body de la función para referirse a cada argumento. También debe devolver un Frame.access para el static link, como primer elemento de la lista.
...
struct
    ...
    let
        ...
        in
            ...
            let
                val r = !nextfree
            in
                (nextfree := !nextfree + (n*Frame.wSz); r)
            end
        end

        (* tabla de labels -> direcciones *)
        local
            val tabLabels: (Temp.label, int) Tabla ref = ref (tabNueva())
        in
        ...
        let
            ...
            val l = (mem, siz)::(List.tabulate(siz, (fn x => (mem+Frame.wSz*(x+1), init))))
   ...
   val _ = print("Size:"^makestring arr^"\n")
        ...
        let
            ...
            val addrs = List.tabulate(ctos, (fn x => mem + x*Frame.wSz))
        ...
        let
            val str = loadString (strPtr (*+ Frame.wSz*)) (* esto era strPtr *)
            val _ = print(str)
        in
            0
        end
        ...
        val tabLib: (Temp.label, int list -> int) Tabla =
        ...
        and evalFun(f, args) =
            let
                ...
                val _ = if showdebug then 
                           (print((Frame.name frame)^":\n");
                            List.app (print o Tigerit.tree) body;
                            print("Argumentos: ");
                ...
                val _ = map (fn (x,y) => 
                    case x of
                      ...
                      | _ => raise Fail "No debería pasar. Interp formalsValues") formalsValues
    ...
    in (print("Comienzo de ejecución...\n"); evalFun("_Main", []); print("Fin de ejecución.\n")) end
end

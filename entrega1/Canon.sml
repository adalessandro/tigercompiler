...
fun basicBlocks stms = 
    let val done = Temp.newlabel()
        ...
        | blocks(stms, blist) =
            blocks(LABEL(Temp.newlabel())::stms, blist)
...
fun canonize l = 
    let 
        ...
        fun canon2 [] = []
          | canon2 (x::xs) =
                case x of
                     (Frame.STRING s) => (Frame.CSTRING s)::(canon2 xs)
                   | (Frame.PROC {body=tb,frame=fr}) => (Frame.CPROC {body=canon tb,frame=fr})::(canon2 xs)
    in
        canon2 l
    end
...
fun splitcanon [] = ([],[])
  | splitcanon (x::xs) = 
        ...
        in  case x of
                 (Frame.CSTRING s) => (ls, s::rs)
               | (Frame.CPROC {body,frame}) => ((body, frame)::ls, rs) 
        end
